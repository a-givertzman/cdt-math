...
impl HoistTackleEfficienyCoeff {
    ...
    pub fn eval(&mut self,mut hoist_tackle_multiplicity: HoistTackleMultiplicity, user_select: &mut Storage, user_load_device: &mut UserLoadHandDevice) -> Result<f64,StrErr> {
        match self.value {
            ...
            None => {
                ...
                let hoist_tackle_multi_value = hoist_tackle_multiplicity.eval(&mut RopesCount::new(), &mut HoistingTackle::new(), user_select, user_load_device).expect(&format!("{}.eval | Error to calculate hoisting tackle multiplicity",self.dbgid));

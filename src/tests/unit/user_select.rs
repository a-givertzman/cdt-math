...
mod user_select {
    use crate::kernel::{
        crane_constructor::user::{
            self,
            user_select::{self, UserSelect},
        },
    ...
    fn test_get_fmg() {
        ...
        let test_data_1 = [
            ...
            (
                ("тип грузозахватного органа механизма подъёма/"),
                "съёмный электрогидравлический грейфер",
            ),
        ];
        ...
        let result_2 = [user.vhmax, user.vhcs, user.cargo_weight, user.m_to_lift];

        let mut counter = 0;
        for (value, target) in test_data_1.into_iter() {
            assert!(
                result_1[counter] == target,
                "result: {:?}\ntarget: {:?}",
                result_1[counter],
                target
            );
